    ...
    with col_prog:
        ...
        def render_steps(active_idx):
            for i, (d, s) in enumerate(zip(dots, placeholders)):
                ...
                text_color = "#e2e8f0" if i <= active_idx else "#475569"
                d.markdown(
                    f'<div class="progress-step">'
                    f'<span style="color:{color}; font-size:0.9rem">{icon}</span>'
                    f'<span style="color:{text_color}">{s}</span>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
...
score_color = (
    "#ef4444" if score < 40 else
    "#f97316" if score < 60 else
    "#eab308" if score < 75 else
    "#22c55e"
)
...
with col1:
    ...
    <div style="background:#0f1626; border:1px solid #1e2d4a; border-radius:14px; padding:1.5rem; text-align:center;">
        ...
        <div style="height:6px; background:#1e2d4a; border-radius:3px; overflow:hidden;">
            <div style="height:100%; width:{score}%; background:{score_color}; border-radius:3px;"></div>
        </div>
        <div style="font-size:0.75rem; color:#64748b; margin-top:0.75rem">{score_label}</div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    ...
    c2.metric("Covered", str(covered))
    c3.metric("Critical Gaps", str(critical_ct))
    c4.metric("High Gaps", str(high_ct))
...
with tab2:
    ...
    if not matrix:
        ...
        for row in matrix:
            ...
            pill_class = {
                "Covered": "pill-covered",
                "Partial": "pill-partial",
                "Missing": "pill-missing",
            }.get(status, "pill-missing")
            rows_html += f"""
            <tr>
    ...
    if not plan:
        ...
        for i, item in enumerate(plan, 1):
            ...
            with st.expander(f"**{i}. {item.get('action', 'Action')}**"):
                ...
                c1.markdown(
                    f"**Effort:** <span style='color:{effort_col}'>{effort}</span>",
                    unsafe_allow_html=True,
                )
                c2.markdown(
                    f"**Impact:** <span style='color:{impact_col}'>{impact}</span>",
                    unsafe_allow_html=True,
                )
